  ...
  async (accessToken, refreshToken, profile, done) => {
    try {
      ...
      if (existing.length > 0) {
        await runQuery(
          `MATCH (u:User { email: $email }) SET u.googleId = $googleId, u.avatar = $avatar`,
      ...
      // New user — send to complete-profile to choose role
      return done(null, { isNewUser: true, googleId, name, email, avatar })
    } catch (err) {
