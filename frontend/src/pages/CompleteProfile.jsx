...
export default function CompleteProfile() {
  ...
  // Read Google profile from query params
  ...
  // If no googleId in params, redirect to login
  useEffect(() => {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true)
    try {
      const data = await api.completeGoogleSignup({
        googleId,
        name,
        email,
        avatar,
  ...
  const ROLES = [
    {
      ...
      icon:  '🌱',
      label: 'Farmer',
      desc:  'Submit farm assessments and view your FarmScore history.',
      secret: false,
    },
    {
      val:    'lender',
      icon:   '🏦',
      label:  'Lender / SACCO',
      desc:   'Review farmer scores and make credit decisions.',
      secret: true,
      secretLabel: 'Lender secret key',
      secretHint:  'Provided by your FarmScore administrator.',
    },
    {
      val:    'admin',
      icon:   '⚙️',
      label:  'Administrator',
      desc:   'Manage users, lenders, and system settings.',
      secret: true,
      secretLabel: 'Admin secret key',
      secretHint:  'System admin secret — contact the platform owner.',
    },
  ]
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-[#0E1525] border border-[#1E2D45] rounded-2xl p-8">
          ...
          <div className="flex items-center gap-3 bg-[#080D1A] border border-[#1E2D45] rounded-xl px-4 py-3 mb-6">
            ...
            <span className="text-[10px] bg-blue-500/20 text-blue-400 border border-blue-500/30 px-2 py-0.5 rounded-full font-bold flex-shrink-0">
              Google
            </span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <button key={r.val} type="button" onClick={() => { setRole(r.val); setSecret('') }}
                    className={`w-full flex items-start gap-3 p-4 rounded-xl border text-left transition-all ${
                      role === r.val
                        ...
                        : 'border-[#1E2D45] hover:border-[#2D6A4F] bg-[#080D1A]'
                    }`}>
                    {/* Radio */}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-bold text-sm ${role === r.val ? 'text-white' : 'text-gray-300'}`}>
                          {r.label}
                        </span>
                      </div>
            ...
            {(role === 'lender' || role === 'admin') && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-3 bg-[#080D1A] border border-[#1E2D45] rounded-xl text-white text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15 transition-all placeholder:text-gray-600"
                />
              </div>
            )}

            {/* Secret key (lender + admin) */}
            {selected?.secret && (
              <div>
                <label className="text-sm text-gray-300 font-semibold block mb-1.5">
                  {selected.secretLabel} <span className="text-red-400">*</span>
                </label>
                <input
                  type="password"
                  value={secret}
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3.5 bg-blue-500 hover:bg-blue-600 text-white font-bold text-sm rounded-xl transition-all active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-2">
